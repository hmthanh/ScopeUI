<script lang="ts">
    ...
    // @param {value} - The inputs current value.
    export let value: any = '';

    // @param {type} - Determiness the input type of the textbox.
    ...
    // @param {searchButton} - Determines whether a search button is present when `type` is set to "search".
    export let searchButton = true;

    // @param {revealButton} - Determines whether a password reveal button is present when `type` is set to "password".
    export let revealButton = true;

    // @param {readonly} - Determines whether the textbox can be typed in or not.
    export let readonly = false;

    // @param {disabled} - Controls whether the TextBox is intended for user interaction, and styles it accordingly.
    ...
    // @param {inputElement} - Obtains a bound DOM reference to the TextBoxs input element.
    export let inputElement: HTMLInputElement = null;

    // @param {containerElement} - Obtains a bound DOM reference to the TextBoxs container element.
    export let containerElement: HTMLDivElement = null;

    // @param {buttonsContainerElement} - Obtains a bound DOM reference to the TextBoxs buttons container element.
    export let buttonsContainerElement: HTMLDivElement = null;

    // @param {clearButtonElement} - Obtains a bound DOM reference to the TextBoxs clear button element. Only available if `clearButton` is set to true, `readonly` is set to false, and a `value` is present.
    export let clearButtonElement: HTMLButtonElement = null;

    // @param {searchButtonElement} - Obtains a bound DOM reference to the TextBoxs search button element. Only available if `type` is set to `search`.
    export let searchButtonElement: HTMLButtonElement = null;

    // @param {revealButtonElement} - Obtains a bound DOM reference to the TextBoxs reveal button element. Only available if `type` is set to `password`.
